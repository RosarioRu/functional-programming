...
/* Steps and Plan:
      ...
      2. first loop will be a for loop that loops through all elements of array (i) 
...
function selectSort(array) {     //[2,1,3,5,4]
  for (let i =0; i<array.length-1; i++) { //first itiration: i = 2
    let smallestNumber = array[i]; //we will assume that element at i is smallest until proven otherwise
    for (let afterI = i+1; afterI<array.length; afterI++) { 
      ...
      if (array[i] > array[afterI]) { //first(2>1), sec(2>3), th(2>5), f(2>4)
        smallestNumber = array[afterI]; //reassign value of 1 to smallesNumber
      }   //once this loop is done, we can set smallestNumber to index 0
